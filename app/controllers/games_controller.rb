class GamesController < ApplicationController

  def name_game
    user_given_name = params["my_name"].upcase

    if user_given_name.starts_with?("A")
      output_message = "Your name starts with the first letter of the alphabet!"
    else 
      output_message = "Great."
    end

    render json: {message: "Your name is #{user_given_name}. #{output_message}"}
end

def number_game
  user_number_choice = params["user_number"].to_i
  winning_number = 36
  if user_number_choice < winning_number
    output_message = "You guessed too high, guess again!"
  elsif user_number_choice > winning_number
    output_message = "You guessed too low, guess again!"
  else
    output_message = "You did it, winner! Good for you. Don;t you feel awesome?"
  end

end

